...
- **Public API** — API-key-authenticated endpoints for external forms/automation tools to push contacts in (with automatic list/tag attachment), enroll them into a sequence, or stop their sequences (see [docs/PUBLIC_API.md](docs/PUBLIC_API.md))
...
Push contacts in from an external form or automation tool (Zapier, a custom script, a website contact form) — see **[docs/PUBLIC_API.md](docs/PUBLIC_API.md)** (repo) or the published **[API reference](https://xgeniousllc.github.io/geniousCampaign/#/api-reference)** for authentication, the full endpoint reference, and request/response examples. Keys are created and managed from Settings > API keys in the app.
