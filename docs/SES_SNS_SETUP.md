...
5. **Point SES at the topic.** In the SES console, open the configuration set named by `SES_CONFIGURATION_SET` (or the sending identity, if you're not using a configuration set) and add an **Event destination** of type SNS, subscribed to the `Bounce`, `Complaint`, **and `Delivery`** event types, targeting the topic from step 2. Delivery is easy to skip since bounce/complaint suppression is the more obvious motivation for setting this up at all — without it, `sends.status` never advances to `delivered` and the delivered/bounced breakdown on campaign/sequence detail pages stays permanently incomplete, even though bounce/complaint handling works fine.

6. **Verify end-to-end.** Send a real email to one of SES's [mailbox simulator addresses](https://docs.aws.amazon.com/ses/latest/dg/send-email-simulator.html) — `bounce@simulator.amazonses.com` for a hard bounce, `complaint@simulator.amazonses.com` for a complaint, `success@simulator.amazonses.com` for a clean delivery. Within a few seconds the address should show up in Settings > Suppression list with the matching reason (bounce/complaint), or the `sends` row should flip to `delivered` (delivery).

7. **If delivered/bounced status still isn't updating despite the subscription showing "Confirmed":** check that `sends.providerMessageId` actually matches what SES reports back. SES's SNS payload reports `mail.messageId` as the bare message ID with no wrapping — the app must store that same bare ID at send time (`SesSenderProvider.send()` uses nodemailer's `info.response`, not `info.messageId`, specifically because `info.messageId` wraps it as an RFC822 `<id@region.amazonses.com>` header value that can never match). If this ever regresses, every Bounce/Complaint/Delivery notification will log `No send found for providerMessageId: ...` and `sends.status` will silently never update, while `webhook_deliveries` still shows the notification as received.
