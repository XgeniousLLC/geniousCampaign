...
describe('CampaignsService.send (integration, real DB) — GC-053 pre-send confirmation', () => {
  ...
  beforeAll(async () => {
    ...
    for (let i = 0; i < 5; i++) {
      const [contact] = await drizzle.db
        .insert(contacts)
        .values({ email: `gc053-${i}-${Date.now()}@example.com` })
        .returning();
  ...
  it('blocks a send above the threshold server-side without confirmed:true, and never enqueues a job', async () => {
    const [campaign] = await drizzle.db
      .insert(campaigns)
      .values({ name: 'Unconfirmed large send', templateId, listIds: [listId] })
      .returning();
    ...
    const [reloaded] = await drizzle.db
      .select()
      .from(campaigns)
  ...
  it('proceeds once confirmed:true is explicitly passed', async () => {
    const [campaign] = await drizzle.db
      .insert(campaigns)
      .values({ name: 'Confirmed large send', templateId, listIds: [listId] })
      .returning();
  ...
  it('deletes a draft campaign', async () => {
    const [campaign] = await drizzle.db
      .insert(campaigns)
      .values({ name: 'To delete', templateId, listIds: [listId] })
      .returning();
  ...
  it('refuses to delete a campaign that is currently sending', async () => {
    ...
    await expect(service.remove(campaign.id)).rejects.toThrow(
      'currently sending',
    );
