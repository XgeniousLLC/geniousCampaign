...
const LIST_DEFS = [
  ...
  { name: 'Enterprise Prospects', type: 'static' as const },
  {
    ...
    type: 'dynamic' as const,
    filterDefinition: { lastActivityBefore: '30d' },
  },
];
...
const TEMPLATE_DEFS = [
  { name: 'Welcome Email', subject: 'Welcome to {{contact.company}}!' },
  {
    ...
    subject: "It's here: our biggest release yet",
  },
  {
  ...
  { name: 'Monthly Newsletter', subject: 'This month at a glance' },
  {
    ...
    subject: 'How {{contact.company}} could 2x conversions',
  },
];
...
async function main() {
  ...
  for (const l of LIST_DEFS) {
    const existing = await db.query.lists.findFirst({
      where: (tbl, { eq }) => eq(tbl.name, l.name),
    });
    if (existing) {
  ...
  for (const contact of allContacts) {
    ...
    const shuffledLists = [...listRows]
      .sort(() => Math.random() - 0.5)
      .slice(0, listCount);
    for (const list of shuffledLists) {
      await db
        .insert(schema.contactLists)
        .values({ contactId: contact.id, listId: list.id })
        .onConflictDoNothing();
    }
  }
  ...
  for (const t of TEMPLATE_DEFS) {
    ...
    const [created] = await db
      ...
      .values({
        name: t.name,
        subjectLines: [t.subject],
        previewTextLines: [],
        bodyJson: {
          type: 'doc',
          content: [
            { type: 'paragraph', content: [{ type: 'text', text: t.subject }] },
          ],
        },
        bodyHtml,
        bodyText: t.subject,
  ...
  const campaignNames = [
    ...
    'Summer Sale Blast',
    'Webinar Invite',
    'Feature Spotlight: Automations',
    'Customer Stories Roundup',
    'Beta Access Announcement',
  ];
  const statuses: Array<'draft' | 'sending' | 'sent' | 'failed'> = [
    'sent',
    'sent',
    'sent',
    'sending',
  ...
  for (let i = 0; i < campaignNames.length; i++) {
    ...
    const recipients = [...allContacts]
      .sort(() => Math.random() - 0.5)
      .slice(0, randomInt(15, 40));
    let sentCount = 0;
    let failedCount = 0;

    for (const contact of recipients) {
      const sentAt = new Date(
        createdAt.getTime() + randomInt(0, 2) * 60 * 60 * 1000,
      );
      const roll = Math.random();
      const sendStatus =
        status === 'failed' && roll < 0.4
          ? 'failed'
          : roll < 0.05
            ? 'bounced'
            : roll < 0.08
      ...
      if (Math.random() < 0.42) {
        await db.insert(schema.emailEvents).values({
          ...
          type: 'open',
          createdAt: new Date(
            sentAt.getTime() + randomInt(1, 6) * 60 * 60 * 1000,
          ),
        });
        if (Math.random() < 0.35) {
          await db.insert(schema.emailEvents).values({
            ...
            createdAt: new Date(
              sentAt.getTime() + randomInt(2, 10) * 60 * 60 * 1000,
            ),
          });
        }
      }
      if (Math.random() < 0.02) {
        await db
          ...
          .values({ sendId: send.id, type: 'bounce', createdAt: sentAt });
      }
    }
