...
15. **Tracking domain is fully internal — `VITE_API_BASE_URL`, always, no custom domain concept, decided 2026-07-14; self-host shortcut added 2026-08-21; superseded 2026-09-08.** The original design (`TRACKING_DOMAIN`, a DB-only, DNS-CNAME-verified Settings field, plus a "self-host shortcut" for using the API's own hostname with no DNS check) was **removed entirely** after a production incident: an admin who never completed the manual DNS-verify step had every pixel/click/unsubscribe URL silently degrade to `http://localhost:PORT`, unreachable from any real mail client, with no error anywhere. Sharifur's call: no optional custom-tracking-domain feature at all — one less moving part that can be silently misconfigured. `TrackingService.baseUrl` (`tracking/tracking.service.ts`) now unconditionally uses `VITE_API_BASE_URL` — already-mandatory config (the frontend can't reach the API without it), so tracking works correctly the moment the app is deployed, zero admin action, ever. `TrackingDomainController`, `TrackingDomainField.tsx`, `verifyTrackingDomain()`, and the generic `SettingDef.verifyOnly`/`SettingFieldStatus.verifyOnly` mechanism they were the only user of were all deleted, not just deprecated. **Do not re-add a configurable tracking domain** — if a future feature genuinely needs a "prove you control this DNS" pattern for something else (e.g. a custom sending domain), design it fresh rather than reviving this one; don't make *this* feature's correctness depend on someone remembering a manual step again.

16. **Template-level variants (`templates.parentTemplateId`) were removed 2026-09-08 in favor of step-level `sequence_step_templates`.** A sequence step now links multiple templates directly (many-to-many), and `SequenceRunnerService` picks one uniformly at random per send — this replaced GC-068's "save a shuffle/AI variant as a hidden child template" mechanism and its undocumented "Variant of" retroactive-linking panel entirely, rather than keeping both variant systems alongside each other. Don't reintroduce a template-level parent/child relationship; if a future feature needs template variants again, extend the step-level join table (or campaigns, which still target a single `templateId`), not `templates` itself. Relatedly, `templates.subjectLines`/`previewTextLines` are arrays (one entry picked at random per send, independently of each other) resolved via `resolveTemplateContent()` in `@genius-campaign/shared` — it composes `resolvePersonalization`/`resolveSpintax` with the random line-pick as the outer step, preserving invariant 5's personalize-before-spintax ordering. A future third resolution layer should follow the same outermost-random-pick-first, then-personalize-then-spintax ordering, and use `resolveTemplateContent()` rather than re-implementing the pattern at a fourth call site.
...
# Tracking
# No TRACKING_DOMAIN — pixel/click/unsubscribe links always use
# VITE_API_BASE_URL above (invariant 15). Nothing else to configure here.
